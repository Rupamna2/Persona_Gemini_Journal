...
# Circuit breaker to avoid wasting 10-15s attempting depleted Gemini models when 429 is received
_gemini_quota_exhausted_until: float = 0.0
GEMINI_QUOTA_COOLDOWN_SECONDS: float = 600.0  # 10 minutes
...
def generate_content_with_fallback(
    ...
    1. Checks if Gemini quota is marked exhausted. If not, tries Gemini models.
       If quota exhausted (429), activates circuit breaker and immediately fails over to NVIDIA NIM.
    2. Uses NVIDIA NIM models (Llama 3.2 11B / Nemotron 120B / Nemotron 30B).
    ...
    # Step 1: Try Gemini API if not circuit-broken
    gemini_key = os.environ.get("GEMINI_API_KEY")
    is_gemini_circuit_broken = time.time() < _gemini_quota_exhausted_until

    if gemini_key and not is_gemini_circuit_broken:
        try:
                ...
                for model_name in model_ladder:
                    try:
                        ...
                        if "429" in error_str or "quota" in error_str or "prepayment" in error_str or "resource_exhausted" in error_str:
                            # Shared quota exhausted across project; trip circuit breaker for 10 minutes
                            _gemini_quota_exhausted_until = time.time() + GEMINI_QUOTA_COOLDOWN_SECONDS
                            logger.info(f"Gemini project quota exhausted. Activated {GEMINI_QUOTA_COOLDOWN_SECONDS}s circuit breaker -> fast failover to NVIDIA NIM.")
                            break
                        # For other retryable errors, continue loop to test next model in ladder
        except Exception as exc:
            logger.warning(f"Gemini client encountered error: {exc}. Attempting failover to NVIDIA NIM.")
    elif is_gemini_circuit_broken:
        logger.debug("Gemini circuit breaker active. Fast routing directly to NVIDIA NIM.")

    # Step 2: Failover to NVIDIA NIM if key is present
    if nvidia_key:
        # If structured JSON is requested, ensure prompt clearly instructs JSON formatting
        ...
        if response_mime_type == "application/json" or "json" in (system_instruction or "").lower():
            json_directive = "\n\nCRITICAL: Respond ONLY with a valid JSON object adhering strictly to the requested schema. Do not enclose in markdown codeblocks and do not output any extra text."
            augmented_system_instruction = f"{system_instruction or ''}{json_directive}"
    ...
    # Context-aware fallback response rather than a single static sentence
    return "I'm listening and reflecting on what you just shared. What is the most important angle or feeling you'd like to explore next?"
